...
describe("Should ", () => {
    it("something happens when clicking the register button", () => {
      ...
      it("kommentar skrivs ut på rätt event när man skrivit ett och tryckt på skicka knappen", () => {
        const wrapper = mount(<MeetupItem id={'0123456'} name={'SpaceX'} date={"2021-02-26"} description={"SpaceX is already a success, with people sent into space. This meetup will feature a lecture about how the program plans to build bases on the Moon and on March."} comments={[]} addComment={(id, comment) => addComment(id, comment)} />)
        const registerBtn = wrapper.find('[data-test="meetups-input"]')
        registerBtn.simulate('click')
